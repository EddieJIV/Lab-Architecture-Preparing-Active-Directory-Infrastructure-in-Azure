...
- Once you've done so, we don't have to worry about anything else for the VNet so we can just click Review + Create!
...
- Navigate to the Compute Infrastructure | Virtual machines section of Azure & click Create, then select Virtual Machine.
...
- Keep everything following region as-is until you get down to image.
...
- Once the DC-1 is created, head back to the Compute infrastructure | Virtual machines for the next step.
...
- In order to do so, navigate to DC-1's home page in Compute infrastructure | Virtual machines.
...
---
Log in to the DC-1 VM for the next Step:

*Please note that disabling the Windows Firewall is not recommended in a real-world environment, as it exposes the system to potential security risks. However, for the purpose of this lab, we are temporarily turning it off to simplify network connectivity and avoid troubleshooting firewall issues, so we can focus on learning Active Directory concepts. Thanks for your understanding!* 
...
- Navigate to Client 1's Network Settings.
...
---
Once the VM has been successfully restarted, use Remote Desktop Connection to proceed to log into Client-1.
...
- If you want to verify you are on the correct Virtual Machine prior to testing the VM's connectivity, you may open up the settings, type "About" into the search bar, and verify that the VM is running on Windows 10 Pro. 
...
- If the ping is successful, it means the network and firewall settings are correctly configured and basic network communication needed for domain services is working.
